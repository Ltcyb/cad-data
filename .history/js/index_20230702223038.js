				...
				$(".programs")
					...
					.each((i, e) => {
						...
						programs.push({
							...
							level: prgm2[0],
							website: $(e).find("a:nth(2)").attr("href"),
							language: prgm1[0],
							coop: prgm1[1],
							distanceEdu: prgm1[2],
						});
				...
				.each((i, e) => {
					...
					if (i === 0) {
						...
						prgm2 = $(e)
							...
							.replace("Graduatelevelcertificate", "Graduate")
							.match(/[A-Z][a-z]+/g);
					}
				});

			console.log("webiste: " + $(e).find(".program_detail").text());

			programs.push({
				name: $(e).find(".toggle-view").text(),
				level: prgm2[0],
				website: $(e).find(".program-details a").attr("href"),
				language: prgm1[0],
				coop: prgm1[1],
				distanceEdu: prgm1[2],
			});
...
test(
	"https://www.universitystudy.ca/search-programs/?k=*&institution_name=Athabasca%20University"
);
